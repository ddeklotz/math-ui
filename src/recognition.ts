...
export const dtw_segment = (left: Stroke, right: Stroke) => {
  ...
  const dtw_idx = (idx_l: number, idx_r: number) =>
    (left.length + 1) * (idx_r + 1) + (idx_l + 1);
...
export const dtw_glyph = (left: Glyph, right: Glyph): number => {
  ...
  for (
    let i = 0;
    i < Math.max(left.strokes.length, right.strokes.length);
    i++
  ) {
    sum += dtw_segment(
      left.strokes[i] ?? emptyStroke,
      right.strokes[i] ?? emptyStroke,
    );
  }
  return sum;
};
