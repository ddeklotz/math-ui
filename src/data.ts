...
export const generateSplitGlyphs = (originalGlyphs: Glyph[], examplesOfEach: number): Glyph[] => {
  const result: Glyph[] = [];
  const glyphsByClass = orderGlyphsByClass(originalGlyphs);
    ...
    for (const rightClass of classes) {
      ...
      for (let i = 0; i < examplesOfEach; ++i) {
        ...
        if (leftGlyph && rightGlyph) {
          ...
          const translatedEndStroke = applyToPoints(translate(-rightBox.x + (leftBox.x + leftBox.width) + .2, -rightBox.y + leftBox.y), endStroke);
            
          result.push({character: /*`not ${leftClass} or ${rightClass}`*/ "none", writer: leftGlyph.writer, repetition: 0, strokes: [startStroke, translatedEndStroke]});
        }
      }
    }
  }
...
export const stroke_findTopLeft = (stroke: [number, number][]): [number, number] => {
  return [
...
export const stroke_findBottomRight = (stroke: [number, number][]): [number, number] => {
  return [
...
export const findBoundingBox = (stroke: [number, number][]): {x: number, y: number, width: number, height: number} => {
...
export const findGlyphBoundingBox = (glyph: Glyph): {x: number, y: number, width: number, height: number} => {
...
export const findDisplayBoundingBox = (glyph: Glyph): {x: number, y: number, s: number} => {
